...
/** Horizontal image carousel for a hotel */
        ...
        <div className="relative group/carousel">
            ...
            >
                {images.map((url, idx) => (
                    <div
                        key={idx}
                        className="relative flex-shrink-0 w-52 h-36 rounded-xl overflow-hidden bg-gray-100"
                    >
                        <Image
                            src={url}
                            alt={`${name} — foto ${idx + 1}`}
...
function RatingBadge({ rating, reviewsCount }: { rating: number; reviewsCount: number }) {
    ...
    return (
        <div className="flex items-center gap-2">
            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-white text-xs font-bold ${bgColor}`}>
        ...
        <div className="flex flex-wrap gap-1.5">
            {amenities.slice(0, 6).map((amenity, i) => {
                ...
                return (
                    <span
                        key={i}
                        className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-gray-50 text-[10px] font-medium text-gray-500"
                    >
...
/** Single hotel option row */
function HotelOptionCard({ hotel }: { hotel: AccommodationOption }) {
    return (
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-sm transition-shadow">
            ...
            <div className="p-4 space-y-3">
                {/* Header: name + type + stars */}
                <div className="flex items-start justify-between gap-2">
                    <div className="space-y-1 min-w-0">
                        <div className="flex items-center gap-2">
                            <h4 className="text-sm font-bold text-[#2D2840] truncate">
                                {hotel.name}
                            </h4>
...
export function AccommodationCard({ options }: AccommodationCardProps) {
    ...
    return (
        <div className="w-full bg-white rounded-2xl shadow-sm border border-purple-200 overflow-hidden">
            {/* Header strip */}
            ...
            <div className="px-4 py-4 space-y-4">
                ...
                <HotelOptionCard hotel={bestOption} />
                ...
                {visibleMore.length > 0 && (
                    <div className="space-y-3 pt-2 border-t border-gray-100">
                        ...
                        {visibleMore.map((hotel, i) => (
                            <HotelOptionCard key={i} hotel={hotel} />
                        ))}
                    </div>
                )}
