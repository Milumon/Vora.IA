    ...
    return (
        <div className="flex items-center gap-3">
            <div className="text-center min-w-[56px]">
                <p className="text-xl font-bold text-[#1a1a2e] leading-none">{transfer.hora_salida}</p>
                <p className="text-[10px] text-gray-400 mt-0.5 truncate max-w-[56px]">{transfer.origen}</p>
            </div>
            ...
            <div className="text-center min-w-[56px]">
                <p className="text-xl font-bold text-[#1a1a2e] leading-none">{transfer.hora_llegada}</p>
                <p className="text-[10px] text-gray-400 mt-0.5 truncate max-w-[56px]">{transfer.destino}</p>
            </div>
        </div>
    );
}
            ...
            <div className="space-y-1.5">
                {extras.map((opt, i) => (
                    <div key={i} className="flex items-center justify-between text-xs text-gray-500">
                        <span className="font-medium text-gray-700 truncate max-w-[120px]">{opt.empresa}</span>
                        <span className="mx-2">
                            {opt.hora_salida} → {opt.hora_llegada}
                        </span>
                        <span className="font-semibold text-gray-800 shrink-0">{formatPrice(opt.precio)}</span>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="w-full bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden">
            ...
            <div className="px-5 py-4 space-y-4">
                <div className="flex items-center justify-between gap-2">
                    <p className="text-sm font-bold text-[#2D2840] truncate">{transfer.empresa}</p>
                    <span
